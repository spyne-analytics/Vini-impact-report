...
function RooftopReport({ data, name, insights, onBackToGroup }: RooftopReportProps) {
  ...
  const salesSections = ["Sales Inbound", "Sales Outbound"].filter((s) => data.sectionData[s]);
  const serviceSections = ["Service Inbound", "Service Outbound"].filter((s) => data.sectionData[s]);
